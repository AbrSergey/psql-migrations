    ...
    return async (client) => {
        try {
            ...
            for (const migration of migrationsToRun) {
                ...
                psqlMigrationLogger.info(`migration: ${migration.id} ${migration.name}`);
                console.log("\x1b[33m%s\x1b[0m", `[postgres-migrate-module]: migration: ${migration.id} ${migration.name}`);
